    ...
    public partial class ПодтверждениеПеремещения
    {
        protected override bool Handling()
        {
            ...
            else
            {
                ...
                остаткиНоменклатуры.New();
                var учетИнстр = РегистрУчетИнструмента.Новый(this);
                var учетИнстрОстатки = РегистрУчетИнструмента.Новый();
                учетИнстр.New();
                ...
                foreach (var i in this.Таблица.AllRows)
                {
                    ...
                    {
                        учетИнстрОстатки.New();
                        учетИнстрОстатки.СерийныйНомер = i.СерийныйНомер;
                        if (учетИнстрОстатки.GetRest(ДатаДокумента.AddSeconds(-2)) && учетИнстрОстатки.ДокументПодтверждения.Selected)
                        {
                            учетИнстр.Сотрудник = Сотрудник;
                            учетИнстр.НоменклатураИнструмента = i.Номенклатура;
                            учетИнстр.Объект = Объект;
                            учетИнстр.Фотография = i.Фотография.Selected ? i.Фотография: учетИнстрОстатки.Фотография;
                            учетИнстр.СерийныйНомер = i.СерийныйНомер;
                            учетИнстр.Дата = ДатаДокумента;
                            учетИнстр.ДокументПодтверждения = ПодтверждениеПеремещения.Новый();
                            учетИнстр.СотрудникПередавший = СотрудникПередавший;
                            учетИнстр.ОбъектПередавший = ОбъектПередавший;

                            учетИнстр.AddMovement();
                        }
                    }
                    else
                    {
                ...
                return остаткиНоменклатуры.Post() && учетИнстр.Post();
            }
        }
        ...
        private bool ПроведениеПоРегиструУчетИнструмента()
        {
            ...
            foreach (var строка in Таблица.AllRows)
            {
                if (строка.ПолучениеПодтверждено)
                {
                    if (остатки != null)
                    {
                        foreach (var item in остатки.AllRows)
                        {
                            ...
                            {
                                рег.Сотрудник = item[РегистрУчетИнструмента.Names.Сотрудник].ToReferent() as ФизЛица;
                ...
                {
                    if (Отправитель is ФизЛица сотрудникОтправительВСтроку)
                    {
