...
namespace TechControl.Метаданные.Учет
{
    ...
    {
        public decimal ПолучитьЦену(DateTime дата) 
        {
            ...
            if (цена == 0)
            {
                РегистрЦен цены = РегистрЦен.Новый();
                цены.Номенклатура = this;
        ...
        public decimal СтоимостьСУчетомАмортизацииНаДату(DateTime датаОпределенияСтоимости, DateTime датаНачалаЭксплуатации, decimal суммаПоДокументу = 0)
        {
            decimal сумма = 0;
            if (ПланАмортизации.Selected && ПланАмортизации.КоличествоПериодов != 0)
            {
                ...
                var суммаБазовая = суммаПоДокументу != 0 ? суммаПоДокументу : ПолучитьЦену(датаНачалаЭксплуатации);
                ...
                while (датаОпределенияСтоимости > датаОкончанияПериода)
                {
                    ...
                    период = период + периодБазовый;
                    датаОкончанияПериода = датаОкончанияПериода.AddMonths((int)периодБазовый);
                }
