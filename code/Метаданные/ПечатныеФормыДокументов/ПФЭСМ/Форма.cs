...

    {
        ...
        private void ЗаполнитьТаблицу()
        {
            ...
            foreach (var item in Таблица.Value.AllRows)
            {
                ...
                if (Объект.Value.ТаблицаТарифы.AllRows.Any(x => x.Тариф == item.Тариф && x.ГруппаСпецТехники == Техника.Value.ГруппаСпецТехники))
                {
                    row[Цена_vmoТаблица].Value = Объект.Value.ТаблицаТарифы.AllRows.FirstOrDefault(x => x.Тариф == item.Тариф && x.ГруппаСпецТехники == Техника.Value.ГруппаСпецТехники).Стоимость;
                }
                row.Post();
            }
