    ...
    public partial class ОтработанныеМесяцы
    {
        public void ЗаполнитьТаблицуФактическимиДаннымиСмен() 
        {
            if (Объект.Selected)
            {
                ...
                HashSet< Tuple<Техника, ФизЛица, Должности>> рабочаяТехника = new HashSet<Tuple<Техника, ФизЛица, Должности>>();
                foreach (var item in сменыТехники)
                {
                    рабочаяТехника.Add(new Tuple<Техника, ФизЛица, Должности>(item.Техника, item.Сотрудник, item.Должность));
                }

                HashSet<Tuple<Должности, ФизЛица>> рабочийПерсонал = new HashSet<Tuple<Должности, ФизЛица>>();

                foreach (var item in сменыПерсонала)
                {
                    рабочийПерсонал.Add(new Tuple<Должности, ФизЛица>(item.Должность, item.Сотрудник));
                }
