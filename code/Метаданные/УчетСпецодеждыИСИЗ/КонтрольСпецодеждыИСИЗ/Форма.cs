...
namespace TechControl.Метаданные.УчетСпецодеждыИСИЗ
{
...

    {
        ...
        private void СформироватьОтчетПоСпецодежде() 
        {
            ...
            cmpРег.Add(УчетСпецодеждыИСИЗ.Names.Сотрудник, всеСотрудники, NsgComparison.In);
                ...
                if (сотрудник.Должность.Selected)
                {
                    foreach (var срокаТаблицыКомплектов in сотрудник.Должность.ТаблицаКомплектовСпецодежды.AllRows)
                    {
                        foreach (var item in срокаТаблицыКомплектов.КомплектСпецодежды.ТаблицаКомплектаСпецодежды.AllRows)
                        {
                            ...
                            var остаткиПоСотруднику = остатки.AllRows.Where(x => (x[УчетСпецодеждыИСИЗ.Names.НоменклатураСпецодеждыИСИЗ].ToReferent() as Номенклатура) == ном
                                 && (x[УчетСпецодеждыИСИЗ.Names.КомплектСпецодежды].ToReferent() as КомплектыСпецодежды) == компл).ToArray();
                            if (остаткиПоСотруднику.Length > 0)
                            {
                                if (сотрудник.РазмерНоменклатуры(ном) != Размеры.Новый())
                                {
                                    for (int i = 0; i < item.Количество; i++)
                                    {
                                        ...
                                        {
                                            row[ДатаВыдачи_vmoДанныеДляОтчета].Value = строкаОстатков[УчетСпецодеждыИСИЗ.Names.ДатаВыдачи].ToDateTime();
                                            row[ТребуетсяВыдача_vmoДанныеДляОтчета].Value = строкаОстатков[УчетСпецодеждыИСИЗ.Names.ДатаВыдачи].ToDateTime().AddDays((int)ном.СрокЭксплуатации) < DateTime.Now;
                                ...
                                {
                                    остаткиПоСотруднику = остаткиПоСотруднику.OrderByDescending(x => x[УчетСпецодеждыИСИЗ.Names.ДатаВыдачи].ToDateTime()).ToArray();
                                    var row = vmoДанныеДляОтчета.Data.MemoryTable.NewRow();
                                    row[Номенклатура_vmoДанныеДляОтчета].Value = ном;
                                    row[Комплект_vmoДанныеДляОтчета].Value = компл;
                                    ...
                                    row[Сотрудник_vmoДанныеДляОтчета].Value = сотрудник;
                                    row[Должность_vmoДанныеДляОтчета].Value = сотрудник.Должность;
                                    row[ДатаВыдачи_vmoДанныеДляОтчета].Value = остаткиПоСотруднику.First()[УчетСпецодеждыИСИЗ.Names.ДатаВыдачи].ToDateTime();
                                    row[ТребуетсяВыдача_vmoДанныеДляОтчета].Value = остаткиПоСотруднику.First()[УчетСпецодеждыИСИЗ.Names.ДатаВыдачи].ToDateTime().AddDays((int)ном.СрокЭксплуатации) < DateTime.Now;
                                    row.Post();
            ...
            if (остатки.Count > 0)
            {
                ...
                {
                    var сотрудник = item[УчетСпецодеждыИСИЗ.Names.Сотрудник].ToReferent() as Сотрудники;
                    var ном = item[УчетСпецодеждыИСИЗ.Names.НоменклатураСпецодеждыИСИЗ].ToReferent() as Номенклатура;
                    var комплект = item[УчетСпецодеждыИСИЗ.Names.КомплектСпецодежды].ToReferent() as КомплектыСпецодежды;
                    ...
                    row[Сотрудник_vmoДанныеДляОтчета].Value = сотрудник;
                    row[Должность_vmoДанныеДляОтчета].Value = сотрудник.Должность;
                    row[ДатаВыдачи_vmoДанныеДляОтчета].Value = item[УчетСпецодеждыИСИЗ.Names.ДатаВыдачи].ToDateTime();
                    row[ТребуетсяВыдача_vmoДанныеДляОтчета].Value = item[УчетСпецодеждыИСИЗ.Names.ДатаВыдачи].ToDateTime().AddDays((int)ном.СрокЭксплуатации) < DateTime.Now;
                    row[ТребуетсяВыдача_vmoДанныеДляОтчета].Value = комплект.Selected && item[УчетСпецодеждыИСИЗ.Names.ДатаВыдачи].ToDateTime().AddDays((int)ном.СрокЭксплуатации) > DateTime.Now;
                    row.Post();
                }
            }
