        ...
        protected override bool Handling()
        {
            ...
            if (Отправитель is Сотрудники сотрудникОтправитель)
            {
                ...
                foreach (var строка in Таблица.AllRows)
                {
                    ...
                    {
                        var nom = item[УчетСпецодеждыИСИЗ.Names.НоменклатураСпецодеждыИСИЗ].ToReferent() as Номенклатура;

                        if (строка.НоменклатураСпецодеждыИСИЗ == nom)
                        {
                            for (int i = 0; i < строка.Количество; i++)
                            {
                                var номер = item[УчетСпецодеждыИСИЗ.Names.УникальныйНомер].ToString();
                                if (!уникальныеНомера.Contains(номер))
                                {
                                    рег.Сотрудник = сотрудникОтправитель;
                                    рег.КомплектСпецодежды = item[УчетСпецодеждыИСИЗ.Names.КомплектСпецодежды].ToReferent() as КомплектыСпецодежды;
                                    рег.НоменклатураСпецодеждыИСИЗ = nom;
                                    рег.ДатаВыдачи = item[УчетСпецодеждыИСИЗ.Names.ДатаВыдачи].ToDateTime();
                                    рег.КонецПериода = ДатаДокумента;
                                    рег.УникальныйНомер = номер;
                                    рег.Размер = item[УчетСпецодеждыИСИЗ.Names.Размер].ToReferent() as Размеры;
                                    рег.AddMovement();
