...
namespace TechControl.Метаданные.Мониторинг
{
    ...
    [NsgTypeName("NsgReportObject", Guid = "fc456d0f-e0fb-4b78-b4b2-451fb6b420d0")]
    public partial class ОтчетЗакрытиеСмены : NsgSoft.DataObjects.NsgReportObject
    {
        #region Данные
        ...
        /// </summary>
        protected ОтчетЗакрытиеСмены(NsgObject obj)
            : base(obj)
        {
        }
        ...
        public static ОтчетЗакрытиеСмены Новый()
        {
            NsgBaseObject obj = CreateByGuid(NsgService.StringToGuid("fc456d0f-e0fb-4b78-b4b2-451fb6b420d0"));
            if (obj == null)
                obj = new ОтчетЗакрытиеСмены();
            return obj as ОтчетЗакрытиеСмены;
        }
	...
	Идентификатор.Guid = NsgService.StringToGuid("41379016-dc60-46e2-99a4-4431f94ce333");
	...
	Автоинкремент.Guid = NsgService.StringToGuid("b41c5078-4587-4700-a8b6-22fa5bd84077");
			...
			#region создание TechControl.Метаданные.Мониторинг.ОтчетЗакрытиеСменыФорма Форма
			{  
	...
	Форма.FormClassName = "TechControl.Метаданные.Мониторинг.ОтчетЗакрытиеСменыФорма";
	...
	Форма.GroupName = "";
	Форма.Guid = NsgService.StringToGuid("29952440-9521-44bf-b816-9c4e10a73a77");
	
				if (!ObjectList.Contains("Форма"))
                    ObjectList.Add(Форма);
			}
			#endregion //создание TechControl.Метаданные.Мониторинг.ОтчетЗакрытиеСменыФорма Форма
			

			#region заполнение полей класса
				//NsgReportObject
	IsLoadedFromDll = true;
	Name = "ОтчетЗакрытиеСмены";
	Visible = true;
	Guid = NsgService.StringToGuid("fc456d0f-e0fb-4b78-b4b2-451fb6b420d0");
	DefaultSortingName = "";
	...
	TableName = "_report_OtchetZakrytieSmeny";
	...
	GroupName = "Мониторинг";
        ...
        public override NsgMultipleObjectDescriptor Descriptor
        {
            get
            {
                if (descriptor == null)
                {
                    ...
                    descriptor.Name = "ОтчетЗакрытиеСмены";
        ...
        {
            return "Мониторинг.ОтчетЗакрытиеСмены";
        }
        ...
        private ОтчетЗакрытиеСмены[] convertArray(NsgMultipleObject[] array)
        {
            ОтчетЗакрытиеСмены[] res = new ОтчетЗакрытиеСмены[array.Length];
            array.CopyTo(res, 0);
            return res;
        }
        ...
        public new virtual ОтчетЗакрытиеСмены[] FindAll(NsgCompare compare)
        {
        ...
        /// <returns>Список найденных объектов.</returns>
        public new virtual ОтчетЗакрытиеСмены[] FindAll(ref int count, Int64 row_start, NsgSorting sorting, NsgCompare compare)
        {
        ...
        /// <returns>Список найденных объектов.</returns>
        public new virtual ОтчетЗакрытиеСмены[] FindAll(ref int count, Int64 row_start, NsgSorting sorting, string inName, object inValue)
        {
        ...
        /// <param name="inValue">Значение для поиска. Условие поиска - полное совпадение.</param>
        /// <returns>Список найденных объектов.</returns>
        public new virtual ОтчетЗакрытиеСмены[] FindAll(ref int count, Int64 row_start, NsgSorting sorting, NsgDataFixedFields inName,
            object inValue)
        {
            return convertArray(base.FindAll(ref count, row_start, sorting, inName, inValue));
        }
        ...
        public override NsgObject CloneObject
        {
            get
            {
                return new ОтчетЗакрытиеСмены(this as NsgObject);
            }
        }
        ...
        public new class ColumnDescriptor : NsgSoft.Forms.NsgColumnDescriptor
        {
            public ColumnDescriptor()
                : base()
            {
                base.NSGType = typeof(ОтчетЗакрытиеСмены);
            }
			...
			public new ОтчетЗакрытиеСмены Value
            {
                get
                {
                    return (ОтчетЗакрытиеСмены)base.Value;
                }
                set
                {
                    base.Value = value;
