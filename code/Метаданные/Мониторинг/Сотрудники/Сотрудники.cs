...
namespace TechControl.Метаданные.Мониторинг
{
    ...
    public partial class ФизЛица
    {
        protected override List<Guid> BasePost()
        {
            ...
            if (ДокументКонтроляСпецодежды.Selected)
            {
                foreach (var item in ДокументКонтроляСпецодежды.Таблица.AllRows)
                {
                    ...
                    var cmp = new NsgCompare();
                    cmp.Add(МониторингФизЛицаТаблицаРазмеров.Names.ВидСвойствНоменклатуры, вид);
                    cmp.Add(МониторингФизЛицаТаблицаРазмеров.Names.ВидРазмернойСетки, видРазмернойСетки);
                    cmp.Add(МониторингФизЛицаТаблицаРазмеров.Names.Размер, item.Размер);
        ...
        public Размеры РазмерНоменклатуры(Номенклатура номенклатура) 
        {
            ...
            {
                var cmp = new NsgCompare();
                cmp.Add(МониторингФизЛицаТаблицаРазмеров.Names.ВидСвойствНоменклатуры, номенклатура.ВидНоменклатуры);
                var row = ТаблицаРазмеров.FindRow(cmp);
                if (row != null)
                {
                    размер = row.Размер;
        ...
        public bool ДопущенКУправлению(Техника техника) 
        {
            if (техника.Selected)
            {
                var должностиДопущенныеКУправлению = техника.ТаблицаДопущенныеДоУправленияДолжности.AllRows.Where(x => x.Должность.Selected).Select(x => x.Должность).ToArray();
                if (должностиДопущенныеКУправлению.Any(x => x == Должность))
                {
                    return true;
                }
                else 
                {
                    foreach (var item in ТаблицаСовмещаемыхДолжностей.AllRows)
                    {
                        if (должностиДопущенныеКУправлению.Any(x => x == item.Должность))
                        {
                            return true;
                        }
                    }
