...
namespace TechControl.Метаданные.Мониторинг
{
    ...
    public partial class Объекты
    {
        ...
        public ФормированиеСмены ПолучитьСменуНаДату(DateTime дата) 
        {
            ...
            {
                var длительностьСмены = МаксДлительностьСменыЧасы != 0 ? МаксДлительностьСменыЧасы : Константы.КонстантыОбщие.ДляВсехПользователей.МаксимальнаяДлительностьСмены;
                if (длительностьСмены == 0)
                {
                    длительностьСмены = 24;
                }
                if (смена.ДатаДокумента.AddHours(длительностьСмены) < DateTime.Now)
                {
                    ФормированиеСменыДень формированиеСмены = ФормированиеСменыДень.Новый();
                    формированиеСмены.New();
                    формированиеСмены.Объект = this;
                    формированиеСмены.Ответственный = Ответственный;
                    формированиеСмены.ДатаДокумента = смена.ДатаДокумента.AddHours(длительностьСмены);
                    формированиеСмены.ДокументОснование = смена;
                    формированиеСмены.Заполнить();
                    формированиеСмены.ЗавершаетСмену = true;
                    формированиеСмены.СтатусДокументаСмены = СтатусыДокументовСмен.АвтоматическиЗавершено;
                    формированиеСмены.Handle();

                    смена = ФормированиеСмены.Новый();
                    смена.New();
                    смена.ДатаДокумента = дата;
                    смена.Объект = this;
                    смена.Ответственный = Ответственный;
